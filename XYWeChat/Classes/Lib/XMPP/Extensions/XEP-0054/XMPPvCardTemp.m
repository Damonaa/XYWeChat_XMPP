...
- (NSArray *)telecomsAddresses {
    ...
    //子节点
    NSXMLElement *number = [tel elementForName:@"NUMBER"];
    
    NSString *telNum = [number stringValue];
    if (telNum.length > 0) {
        return @[telNum];
    }
...
- (void)setTelecomsAddresses:(NSArray *)tels {
    ...
    NSXMLElement *tel = [self elementForName:@"TEL"];
    //删除NUMBER节点
    [tel removeElementForName:@"NUMBER"];
    //添加回去
    NSXMLElement *telNum = [NSXMLElement elementWithName:@"NUMBER" stringValue:tels[0]];
    [tel addChild:telNum];
...
- (NSArray *)emailAddresses {
    //获取email的节点
    NSXMLElement *name = [self elementForName:@"EMAIL"];
    //获取子节点
    NSXMLElement *useridEml = [name elementForName:@"USERID"];
    
    NSString *mail = [useridEml stringValue];
